...
class LSTMNetwork(nn.Module):
    def __init__(self, input_dim=128, hidden_dim=256 * 3, num_layer=1):
        super().__init__()
        self.input_dim = input_dim
        self.hidden_dim = hidden_dim
    ...
    def init_hidden(self, batch_size):
        self.h = torch.zeros(
            (self.num_layer, batch_size, self.hidden_dim)
        ).cuda()
        self.c = torch.zeros(
            (self.num_layer, batch_size, self.hidden_dim)
        ).cuda()
...
class Decoder(nn.Module):
    def __init__(self, input_dim=128, hidden_dim=512, out_dim=256):
        ...
        self.fc3 = nn.Linear(hidden_dim // 2, out_dim, bias=True)
